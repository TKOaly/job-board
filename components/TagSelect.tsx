...
const CreateTagCommandItem = ({ onCreated, tags }: { tags: Tag[], onCreated: (tag: Tag) => void }) => {
  const [state, setState] = useState('idle');
  const search = useCommandState((state) => state.search);

  if (search === '' || tags.find((t) => t.name.toLowerCase() == search.toLowerCase() )) {
    return null;
  }
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className={twMerge('w-[200px] min-h-10 h-auto justify-between', className)}
        >
            ...
            {allTags.map((tag) => (
              ...
              >
                <CheckIcon
                  className={twMerge(
                    "mr-2 h-4 w-4",
                    value.find((t) => t.id == tag.id) ? "opacity-100" : "opacity-0"
                  )}
                />
                {tag.name}
              </CommandItem>
